...
Serial::SerialImpl::reconfigurePort ()
{
  ...
  //set timeout(add)
  uint32_t bit_time_ns = 1e9 / dcbSerialParams.BaudRate;
  DWORD serialBitsPerByte = dcbSerialParams.ByteSize + 1;
  serialBitsPerByte += (dcbSerialParams.Parity == NOPARITY) ? 0 : 1;
  serialBitsPerByte += (dcbSerialParams.StopBits == ONESTOPBIT) ? 1 : 2;
//  DWORD msPerByte = (dcbSerialParams.BaudRate > 0) ? ((1000 * serialBitsPerByte + dcbSerialParams.BaudRate - 1) /
//                                            dcbSerialParams.BaudRate) : 1;
  byte_time_ns_ = bit_time_ns * serialBitsPerByte;
}
